...
> **주의** : 아래의 스크립트 코드는 별도의 조작 없이 그대로 삽입한다.
...
> **주의** : 아래의 스크립트 코드는 리다이렉트 시 주석 부분에서 리다이렉트 처리를 하는 것을 제외하고는 별도의 조작 없이 그대로 삽입한다.
...
`http://ADdomain.com/event/?user=1009&bz_tracking_id=10023_71ffbffd-ccf1-4edf-9c4c`
...
##### 주의사항
**완료 후 최종 테스트를 BuzzAd 담당자가 진행하게 되는데, 여기서 생성한 url을 다시 BuzzAd 측으로 전달할 필요는 없다**. BuzzAd 담당자가 최종 연동 테스트를 할 때는 이와 달리 실제 환경에서 실제 트래킹 아이디를 이용해서 적립금 지급 테스트를 진행하게 되므로 **기존의 url을 전달해야 한다(bz_tracking_id 가 없는 url)**. 실제 환경에서 bz_tracking_id 는 클릭 시마다 달라지는 값이 들어가며, url에 별도의 파라미터로 bz_tracking_id를 넣을 필요는 없다.

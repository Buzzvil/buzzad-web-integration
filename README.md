...
아래의 테스트용 bz_tracking_id 를 랜딩 url에 붙여서 호출한다.
...
##### 주의사항
테스트용 랜딩 url은 광고주 측의 연동 테스트를 위함이며, 완료 후 최종 테스트를 위해 이 url을 다시 BuzzAd 측으로 전달할 필요는 없다. BuzzAd 담당자가 최종 연동 테스트를 할 때는 이와 달리 실제 환경에서 실제 트래킹 아이디를 이용해서 적립금 지급 테스트를 진행하게 된다.
